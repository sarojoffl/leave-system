...
@login_required
def export_my_leaves_pdf(request):
    today = date.today()
    leaves = (
    ...
    sections = [
        (
            None,
            ["Leave Type", "From", "To", "Days", "Reason", "Decision Note", "Approved On"],
            [
                [
                    l.leave_type.name, l.start_date, l.end_date, l.days,
    ...
    filename = f"my_approved_leaves_{request.user.username}_{today.isoformat()}.pdf"
    return _build_pdf_response(
        filename=filename,
        title=f"Approved Leaves — {request.user.get_full_name() or request.user.username}",
        subtitle=f"Exported on {today}",
        sections=sections,
    )
...
@login_required
def export_my_attendance_pdf(request):
    today = date.today()
    requests = (
    ...
    sections = [
        (
            None,
            ["Date (AD)", "Date (BS)", "Reason", "Decision Note", "Approved On"],
            [
                [
    ...
    filename = f"my_approved_attendance_{request.user.username}_{today.isoformat()}.pdf"
    return _build_pdf_response(
        filename=filename,
        title=f"Approved Attendance Corrections — {request.user.get_full_name() or request.user.username}",
        subtitle=f"Exported on {today}",
        sections=sections,
    )
...
@login_required
def export_my_holiday_work_pdf(request):
    today = date.today()
    requests = (
    ...
    filename = f"my_approved_holiday_work_{request.user.username}_{today.isoformat()}.pdf"
    return _build_pdf_response(
        filename=filename,
        title=f"Approved Holiday Work — {request.user.get_full_name() or request.user.username}",
        subtitle=f"Exported on {today}",
        sections=sections,
    )
